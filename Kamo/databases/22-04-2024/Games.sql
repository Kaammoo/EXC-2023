...
SELECT production_year, COUNT(title) AS count, AVG(production_cost) AS avg_cost, AVG(revenue) AS avg_revenue FROM Games GROUP BY production_year;

SELECT type, company, COUNT(*) AS number_of_games FROM Games WHERE revenue > production_cost GROUP BY type, company;

SELECT company, COUNT(*) AS number_of_games, AVG(production_cost) AS avg_cost FROM Games GROUP BY company HAVING COUNT(*) > 1;
